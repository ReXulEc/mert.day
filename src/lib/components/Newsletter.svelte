...
<div class="flex bg-[#808080]/5 w-full p-6 rounded-3xl gap-10 justify-between">
    <div class="space-y-1.5 flex flex-col md:w-3/4">
        ...
        <div class="flex space-x-2">
            <input on:input={(e) => (data.email = e.target.value.trim())} class="bg-[#4F4F4F] p-3 w-full rounded-lg ring-0 placeholder-white/50 flex text-white" placeholder="email adress" type="email" name="" id="">
            <button on:click={() => (fetching())} class="bg-[#4F4F4F] p-3 px-7 rounded-lg text-white flex">Subscribe</button>
        </div>
    </div>
